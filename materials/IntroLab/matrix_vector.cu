...
__global__ void compute_triad(const int N, const float *A, const float *x, float *y)
{
  ...
  for (unsigned int i = 0; i < chunk_size; ++i)
  {
    ...
    if (row < N)
    {
      float sum = 0.f; // Use a temporary variable to accumulate the sum
      for (int col = 0; col < N; col++){
        sum += A[row + N *  col] * x[col];
      }
      y[row] = sum; // Store the computed sum into y[row] once all contributions are added
    }
  }
}
                     ...
                     const long long   repeat)
{
  ...
  std::cout << "STREAM triad of size " << std::setw(8) << N
            ...
            << 1e-9 * sizeof(float) * N * (2*N + 1) / best << " GB/s" << std::endl;
}
