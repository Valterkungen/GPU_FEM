...
__global__ void compute_triad(const int N, const float *A, const float *x, float *y)
{
  const int idx_base = threadIdx.x + blockIdx.x * (blockDim.x * chunk_size);
  const int idx_base2 = threadIdx.y + blockIdx.y * (blockDim.y * chunk_size);
  for (unsigned int i = 0; i < chunk_size; ++i)
  {
    const int row = idx_base + i * block_size;
    const int col = idx_base2 + i * block_size;
    if (row < N && col <N)
    {
      float sum = 0.f;
      for (int j = 0; j < N; j++){
        sum += A[row + N *j] * x[col * j + N];
      }
      y[row + col * N] = sum;
    }
  }
                     ...
                     const long long   repeat)
{
  ...
  cudaMalloc(&v1, N * N * sizeof(float));
  cudaMalloc(&v2, N * N * sizeof(float));
  cudaMalloc(&v3, N * N * sizeof(float));

  const unsigned int n_blocks = (N*N + block_size - 1) / block_size;

  set_vector<<<n_blocks, block_size>>>(N * N, 17.f, v1);
  set_vector<<<n_blocks, block_size>>>(N * N, 42.f, v2);
  set_vector<<<n_blocks, block_size>>>(N * N, 0.f, v3);
  ...
  for (unsigned int t = 0; t < n_tests; ++t)
    {
      ...
      for (unsigned int rep = 0; rep < n_repeat; ++rep)
        compute_triad<<<n_blocks, block_size>>>(N, v1, v2, v3);
  ...
  if ((result_host[0] + result_host[N-1]) != 1428.f * N)
    std::cout << "Error in computation, got "
              << (result_host[0] + result_host[N-1]) << " instead of " << 1428.f * N 
              << std::endl;
  ...
  std::cout << "STREAM triad of size " << std::setw(8) << N
            ...
            << 1e-9 * (N * N) * (2*N - 1) / best << " GFlops/s" << std::endl;
}
